...
sub FAT_INIT()
	...
	FAT_DirectoryBuffer=MAlloc(FAT_DIRSIZE)
	dim descriptor as FS_DESCRIPTOR ptr
	descriptor=MAlloc(sizeof(FS_DESCRIPTOR))
...
sub FAT_FORMAT(disk as unsigned integer)
end sub

function FAT_SELECT(disk as unsigned integer, parametre as byte ptr)  as FS_RESSOURCE ptr
	ConsoleWrite(@"Probe fat file system")
	dim retval as FAT_FS_RESSOURCE ptr=0
	retval=MAlloc(sizeof(FAT_FS_RESSOURCE))
	retval->disk=disk
	...
	UDevInvoke(disk,0,retval->first_fat_sector,retval->fat_sectors,cuint(retval->fat_table))
...
sub FAT_FS_RESSOURCE.Read_ROOT(dst as FAT_ENTRY ptr)
	if (this.Fat_Type<>32) then
        UdevInvoke(this.Disk,0,this.reserved_sectors+this.fat_sectors,this.root_dir_sectors,cuint(dst))
        'UdevInvoke(this.Disk,0,this.reserved_sectors+this.fat_sectors,this.root_dir_sectors,cuint(dst))
	else
...
sub FAT_FS_RESSOURCE.WRITE_ROOT(dst as FAT_ENTRY ptr)
	if (this.Fat_Type<>32) then
        UDevInvoke(this.Disk,1,this.reserved_sectors+this.fat_sectors,this.root_dir_sectors,cuint(dst))
        'UDevInvoke(this.Disk,1,this.reserved_sectors+this.fat_sectors,this.root_dir_sectors,cptr(byte ptr,dst))
	else
...
sub FAT_FS_RESSOURCE.READ(clusternum as unsigned integer, dst as unsigned byte ptr)
    ...
    while(nxt<this.fat_limit)
        UdevInvoke(this.Disk,0,this.Absolute_sector(nxt)-1,this.sector_per_cluster,cuint(buf))
...
function FAT_WRITEFILE(ressource  as FS_RESSOURCE ptr,fichier as unsigned byte ptr,taille as unsigned integer,buffer as unsigned byte ptr) as unsigned integer
    ...
    dim tmpname as unsigned byte ptr=MAlloc(256)
    dim acreer as unsigned byte ptr=MAlloc(32)
	...
	if (clusternum>0) then
        ...
        if (theFat->IsDirty) then
            UDevInvoke(theFat->Disk,1,thefat->first_fat_sector,thefat->fat_sectors,cuint(thefat->fat_table))
            theFat->IsDirty=0
        end if
...
sub FatFS_WriteSectors(disk as unsigned integer,lba as unsigned integer,nbrBlocs as unsigned integer,buffer as unsigned byte ptr)
    dim i as unsigned integer
    dim j as unsigned integer
    for i=0 to nbrBlocs-1
        UdevInvoke(disk,1,lba+i,1,cuint(buffer)+(i*512))
        for j=0 to 500000:next
    next i
end sub

sub FatFS_ReadSectors(disk as unsigned integer,lba as unsigned integer,nbrBlocs as unsigned integer,buffer as unsigned byte ptr)
    dim i as unsigned integer
    dim j as unsigned integer
    for i=0 to nbrBlocs-1
        UdevInvoke(disk,0,lba+i,1,cuint(buffer)+(i*512))
        'for j=0 to 500000:next
    next i
end sub