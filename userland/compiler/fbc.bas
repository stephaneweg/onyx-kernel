...
#include once "onyx.bas"
declare function filelen(n as string) as unsigned integer
function filelen(n as string) as unsigned integer
    return -1
end function
...
private sub hPrintOptions( byval verbose as integer )
	...
	ConsoleWriteLine(@"usage: fbc [options] <input files>")
	ConsoleWriteLine(@"input files:")
	ConsoleWriteLine(@"  *.a = static library, *.o = object file, *.bas = source")
	ConsoleWriteLine(@"  *.rc = resource script, *.res = compiled resource (win32)")
	ConsoleWriteLine(@"  *.xpm = icon resource (*nix/*bsd)")
	ConsoleWriteLine(@"options:")
	ConsoleWriteLine(@"  @<file>          Read more command line arguments from a file")
	ConsoleWriteLine(@"  -a <file>        Treat file as .o/.a input file")
	ConsoleWriteLine(@"  -arch <type>     Set target architecture (default: 486)")
	ConsoleWriteLine(@"  -asm att|intel   Set asm format (-gen gcc|llvm, x86 or x86_64 only)")
	ConsoleWriteLine(@"  -b <file>        Treat file as .bas input file")
	ConsoleWriteLine(@"  -c               Compile only, do not link")
	ConsoleWriteLine(@"  -C               Preserve temporary .o files")
	ConsoleWriteLine(@"  -d <name>[=<val>]  Add a global #define")
	ConsoleWriteLine(@"  -dll             Same as -dylib")
	ConsoleWriteLine(@"  -dylib           Create a DLL (win32) or shared library (*nix/*BSD)")
	ConsoleWriteLine(@"  -e               Enable runtime error checking")

	if( verbose ) then
	ConsoleWriteLine(@"  -earray          Enable array bounds checking")
	ConsoleWriteLine(@"  -eassert         Enable assert() and assertwarn() checking")
	ConsoleWriteLine(@"  -edebug          Enable __FB_DEBUG__")
	ConsoleWriteLine(@"  -edebuginfo      Add debug info")
	ConsoleWriteLine(@"  -elocation       Enable error location reporting")
	ConsoleWriteLine(@"  -enullptr        Enable null-pointer checking")
	end if

	ConsoleWriteLine(@"  -ex              -e plus RESUME support")
	ConsoleWriteLine(@"  -exx             -ex plus array bounds/null-pointer checking")
	ConsoleWriteLine(@"  -export          Export symbols for dynamic linkage")
	ConsoleWriteLine(@"  -forcelang <name>  Override #lang statements in source code")
	ConsoleWriteLine(@"  -fpmode fast|precise  Select floating-point math accuracy/speed")
	ConsoleWriteLine(@"  -fpu x87|sse     Set target FPU")
	ConsoleWriteLine(@"  -g               Add debug info, enable __FB_DEBUG__, and enable assert()")

	if( verbose ) then
	ConsoleWriteLine(@"  -gen gas         Select GNU gas assembler backend")
	ConsoleWriteLine(@"  -gen gcc         Select GNU gcc C backend")
	ConsoleWriteLine(@"  -gen llvm        Select LLVM backend")
	else
	ConsoleWriteLine(@"  -gen gas|gcc|llvm  Select code generation backend")
	end if

	ConsoleWriteLine(@"  [-]-help         Show this help output")
	ConsoleWriteLine(@"  -i <path>        Add an include file search path")
	ConsoleWriteLine(@"  -include <file>  Pre-#include a file for each input .bas")
	ConsoleWriteLine(@"  -l <name>        Link in a library")
	ConsoleWriteLine(@"  -lang <name>     Select FB dialect: fb, deprecated, fblite, qb")
	ConsoleWriteLine(@"  -lib             Create a static library")
	ConsoleWriteLine(@"  -m <name>        Specify main module (default if not -c: first input .bas)")
	ConsoleWriteLine(@"  -map <file>      Save linking map to file")
	ConsoleWriteLine(@"  -maxerr <n>      Only show <n> errors")
	ConsoleWriteLine(@"  -mt              Use thread-safe FB runtime")
	ConsoleWriteLine(@"  -nodeflibs       Do not include the default libraries")
	ConsoleWriteLine(@"  -noerrline       Do not show source context in error messages")
	ConsoleWriteLine(@"  -noobjinfo       Do not read/write compile-time info from/to .o and .a files")
	ConsoleWriteLine(@"  -nostrip         Do not strip symbol information from the output file")
	ConsoleWriteLine(@"  -o <file>        Set .o (or -pp .bas) file name for prev/next input file")
	ConsoleWriteLine(@"  -O <value>       Optimization level (default: 0)")
	ConsoleWriteLine(@"  -p <path>        Add a library search path")
	ConsoleWriteLine(@"  -pic             Generate position-independent code (non-x86 Unix shared libs)")
	ConsoleWriteLine(@"  -pp              Write out preprocessed input file (.pp.bas) only")
	ConsoleWriteLine(@"  -prefix <path>   Set the compiler prefix path")
	ConsoleWriteLine(@"  -print host|target  Display host/target system name")
	ConsoleWriteLine(@"  -print fblibdir  Display the compiler's lib/ path")
	ConsoleWriteLine(@"  -print x         Display output binary/library file name (if known)")
	if( verbose ) then
	ConsoleWriteLine(@"  -print sha-1     Display compiler's source code commit sha-1 (if known)")
	end if
	ConsoleWriteLine(@"  -profile         Enable function profiling")
	ConsoleWriteLine(@"  -r               Write out .asm/.c/.ll (-gen gas/gcc/llvm) only")
	ConsoleWriteLine(@"  -rr              Write out the final .asm only")
	ConsoleWriteLine(@"  -R               Preserve temporary .asm/.c/.ll/.def files")
	ConsoleWriteLine(@"  -RR              Preserve the final .asm file")
	ConsoleWriteLine(@"  -s console|gui   Select win32 subsystem")
	ConsoleWriteLine(@"  -showincludes    Display a tree of file names of #included files")
	ConsoleWriteLine(@"  -static          Prefer static libraries over dynamic ones when linking")
	ConsoleWriteLine(@"  -strip           Omit all symbol information from the output file")
	ConsoleWriteLine(@"  -t <value>       Set .exe stack size in kbytes, default: 1024 (win32/dos)")
	if( verbose ) then
	'' !!! TODO !!! provide more examples of available targets
	ConsoleWriteLine(@"  -target <name>   Set cross-compilation target")
	else
	ConsoleWriteLine(@"  -target <name>   Set cross-compilation target")
	end if
	ConsoleWriteLine(@"  -title <name>    Set XBE display title (xbox)")
	ConsoleWriteLine(@"  -v               Be verbose")
	ConsoleWriteLine(@"  -vec <n>         Automatic vectorization level (default: 0)")
	ConsoleWriteLine(@"  [-]-version      Show compiler version")
	ConsoleWriteLine(@"  -w all|pedantic|<n>  Set min warning level: all, pedantic or a value")
	if( verbose ) then
	ConsoleWriteLine(@"  -w all           Enable all warnings")
	ConsoleWriteLine(@"  -w none          Disable all warnings")
	ConsoleWriteLine(@"  -w param         Enable parameter warnings")
	ConsoleWriteLine(@"  -w escape        Enable string escape sequence warnings")
	ConsoleWriteLine(@"  -w next          Enable next statement warnings")
	ConsoleWriteLine(@"  -w signedness    Enable type signedness warnings")
	ConsoleWriteLine(@"  -w constness     Enable const type warnings")
	ConsoleWriteLine(@"  -w suffix        Enable invalid suffix warnings")
	end if
	ConsoleWriteLine(@"  -Wa <a,b,c>      Pass options to 'as'")
	ConsoleWriteLine(@"  -Wc <a,b,c>      Pass options to 'gcc' (-gen gcc) or 'llc' (-gen llvm)")
	ConsoleWriteLine(@"  -Wl <a,b,c>      Pass options to 'ld'")
	ConsoleWriteLine(@"  -x <file>        Set output executable/library file name")

	if( verbose ) then
	ConsoleWriteLine(@"  -z gosub-setjmp  Use setjmp/longjmp to implement GOSUB")
	ConsoleWriteLine(@"  -z valist-as-ptr Use pointer expressions to implement CVA_*() macros")
	end if
...
private sub hPrintVersion( byval verbose as integer )
	...
	ConsoleWriteLine(@"FreeBASIC Compiler - built for ONYX")
	ConsoleWriteLine(@"Copyright (C) 2004-2021 The FreeBASIC development team.")
...
sub main( byval argc as integer, byval argv as zstring ptr ptr )
    SlabINIT()
    OnyxInit()
	if( argc < 2 ) then
		hPrintOptions( FALSE )
