...
destructor GImage()
    ...
    'to do : tell the server to destroy the handle
    this._parent = 0
    this._handle = 0
    this._width = 0
    this._height = 0
...
sub GImage.OnMouseClick(s as sub(elem as unsigned integer,param as unsigned integer))
    if (this._handle<>0) then
        GDIOnMouseClick(this._handle,s)
    end if
end sub

sub GImage.Flush()
    if (this._handle<>0) then
        GDIInvalidate(this._handle)
    end if
end sub
...
function GImage.LoadFromRaw(path as unsigned byte ptr,_w as unsigned integer,_h as unsigned integer) as GImage ptr
    ...
    if (fbuff<>0 and fsize<>0) then
        ...
        result->Constructor(&hFFFFFFFF,0,0,_w,_h)
...
Function GImage.LoadFromBitmap(path as unsigned byte ptr) as GImage ptr
    ...
    if (header<>0 and fsize<>0) then
        ...
        dim result as GImage Ptr = cptr(GImage ptr,MAlloc(sizeof(GImage)))
        result->Constructor(&hFFFFFFFF,0,0,header->PixelWidth,header->PixelHeight)
